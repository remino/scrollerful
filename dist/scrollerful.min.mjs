...
var e = (e, t, n) => {
	...
	v = `${n}_style`,
	y,
	b = (e, t = !1) => (t ? e.scrollWidth : e.scrollHeight),
	x = () => document.getElementById(v),
	S = () => document.getElementById(_).getBoundingClientRect(),
	C = e => S()[e ? 'width' : 'height'],
	w = (e, t) =>
		['auto', 'scroll'].includes(
			getComputedStyle(e).getPropertyValue(`overflow-${t ? 'x' : 'y'}`)
		),
	T = (...e) => e.sort((e, t) => e - t),
	E = (e, t, n) => {
		let [r, i] = T(t, n)
	...
	H = () => {
		;(k(),
			O(),
			Array.from(document.querySelectorAll(h)).forEach(e => {
				;(e.addEventListener('resize', B),
					e.addEventListener('scroll', B),
					V(e),
					B({ target: e }))
			}),
			window.addEventListener('resize', () => B({ target: document.body })),
			window.addEventListener('scroll', () => B({ target: document.body })),
			V(document.body),
			B({ target: document.body }),
			D())
	}
//#endregion
export { H as default }
